...
# paths
parser.add_argument('--cuda_path', type=str, default = r"C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v11.3", help = 'path to CUDA')
...
parser.add_argument("--n_chans", type=int, default=19,                                                              help = "number of channels")
parser.add_argument('--n_z', type=int, default=16,                                                                  help = 'latent space size')
parser.add_argument("--in_filters", type=int, default=1,                                                            help = "number of input filters")
parser.add_argument("--out_filters", type=int, default=1,                                                           help = "number of output filters")
parser.add_argument("--factor", type =int, default=1,                                                               help = "factor")
parser.add_argument("--num_map_layers", type = int, default=1,                                                      help = "number of map layers")
...
losses_g = []
i_epoch = 0
z_vars_im = rng.normal(0, 1, size=(args.n_batch, args.n_z)).astype(np.float32) # initialize the z variables
...
for i_block in range(i_block_tmp, args.n_blocks): ################# for blocks
    ...
    for i_epoch in tqdm(range(i_epoch_tmp, args.block_epochs[i_block])): ################## for epochs
        ...
        for idx in range(len(data)):
            ...
            batches = get_balanced_batches(train.shape[0], rng, True, batch_size=args.n_batch) # get the batches
            ...
            for it in range(iters): ##################for iterations
                ...
                #generator training
                z_vars = rng.normal(0, 1, size=(args.n_batch, args.n_z)).astype(np.float32)
    ...
    discriminator.reset_parameters(new_num_steps= args.block_epochs[i_block] * len(data) * args.n_batch)
    generator.reset_parameters(new_num_steps= args.block_epochs[i_block] * len(data) * (args.n_batch / args.n_critic)
