    ...
    for i_epoch in tqdm(range(i_epoch_tmp, args.block_epochs[i_block])):  ################## for epochs
        ...
        for idx in range(len(data)):
            ...
            for it in range(iters):  ##################for iterations
                ...
                for i_critic in range(args.n_critic):  # ############################## for critics
                    ...
                    z_vars = rng.normal(0, 1, size=(len(batches[it * args.n_critic + i_critic]), args.n_z)).astype(
                        np.float32)
                    with torch.no_grad():
        ...
        if i_epoch % 10 == 0:
            ...
            freqs_tmp = np.fft.rfftfreq(train_tmp.numpy().shape[2],
                                        d=1 / (250. / np.power(2, args.n_blocks - 1 - i_block)))
            ...
            plot_stuff(fake_fft, freqs_tmp, i_block, i_epoch, batch_fake, args.model_path, model_name, args.jobid,
                       train_amps)
